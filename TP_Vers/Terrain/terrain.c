...
extern int terrain_afficher( const int fd ) {
     
     int nb_lig = 0 ; 
     int nb_col = 0 ; 
     char cellule = CASE_LIBRE ;
     int l,c ; 
     char mess[256];
...
extern int terrain_initialiser( const char * fich_terrain , int nb_lig , int nb_col ) {

     int fd ;
     int l,c ;
     case_t tcl = CASE_LIBRE ;
     char mess[256] ;
...
extern int terrain_voisins_rechercher( const int fd,		/* descripteur fichier terrain */
			    ...
			    int * nb_voisins )		/* nb de voisins */
{
     ...
     size_t taille_case =  CASE_TAILLE ;
     size_t taille_ligne ;
...
extern int terrain_case_libre_rechercher( const int fd, coord_t * const liste_voisins, const int nb_voisins, int * ind_case_libre ) {
    ...
    case_t case_voisin ;
     ...
     if( read( fd , &case_voisin , CASE_TAILLE) == -1 )
     {
	  perror("terrain_case_libre_rechercher : Pb lecture 1ere case\n");
	  return(ERREUR) ;
     }

     if( case_voisin == CASE_LIBRE )
     {
...
extern int terrain_marque_ecrire( const int fd, const coord_t coord, const case_t marque ) {
...
static int terrain_rech_minmax_x( coord_t * const liste_voisins, const int nb_voisins, int * min, int * max) {
...
static int terrain_voisins_marque_afficher( const int fd, coord_t * const liste_voisins, const int nb_voisins ) {
		 ...
		 (l == liste_voisins[ind_voisin].y ) )
	  {
		      ...
		      ( c <= max_col ) )
	       {
		    ...
		    if( c == liste_voisins[ind_voisin].x ) 
		    {
			 ...
			 read(fd, &case_voisin , CASE_TAILLE );
...
static int terrain_voisins_pos_afficher( coord_t * const liste_voisins, const int nb_voisins ) {
...
extern int terrain_voisins_afficher( const int fd, coord_t * const liste_voisins, const int nb_voisins ) {
...
extern int terrain_dim_lire( const int fd,	/* descripteur fichier terrain */
...
extern int terrain_xy2pos( const int fd, /* descripteur fichier terrain */
		...
		off_t * pos ) /* position dans le fichier terrain */
{
     ...
     (*pos) = TERRAIN_TAILLE_ENTETE + ( y*CASE_TAILLE*nb_col ) + (x*CASE_TAILLE) ;
...
extern int terrain_pos2xy( const int fd,		/* descripteur fichier terrain */
...
extern int terrain_case_lire( const int fd,	/* descripteur du fichier terrain */
		   ...
		   case_t * contenu )	/* contenu de la case a lire */
{
     ...
     if( read( fd , contenu , CASE_TAILLE ) == -1 )
     {
